...
"""Swipe words across the keyboard in the test session and check what gets typed.
    ...
    tools/nested/swipe_test.py DIR [word ...]
...
ROWS = [("qwertyuiop", 407, 60, 729), ("asdfghjkl", 437, 60, 789), ("zxcvbnm", 467, 60, 849)]
KEYS = {c: (x0 + i * step, y) for letters, x0, step, y in ROWS for i, c in enumerate(letters)}
...
def main():
    ...
    for word in words:
        ...
        d.swipe(finger_path(word, rng), seg_s=0.07)
        d.wait(0.6)
